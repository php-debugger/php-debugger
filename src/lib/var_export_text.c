...
static void xdebug_var_export_text_ansi(zval **struc, xdebug_str *str, int mode, int level, int debug_zval, xdebug_var_export_options *options)
{
	...
	switch (z_type) {
		...
		case IS_DOUBLE:
			xdebug_str_add_fmt(str, "%sdouble%s(%s%.*H%s)", ANSI_COLOR_BOLD, ANSI_COLOR_BOLD_OFF, ANSI_COLOR_DOUBLE, (int) PG(serialize_precision), Z_DVAL_P(*struc), ANSI_COLOR_RESET);
			break;
